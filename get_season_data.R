...
# Function to transform and split game data for each team
# byTeamGameData <- function(nflScheduleData, nflTeamsInfo) {
#   if (!("team_abbr" %in% names(nflTeamsInfo))) {
#     stop("Error: 'team_abbr' column not found in team info data")
#   }
...
#     rename(away_team_conf = team_conf, away_team_div = team_division)
#   
#   # Transform the DataFrame to have one row per team game
#   team_games <- reshape_team_games(games_with_conf)
#   
#   # Split the data by team
#   team_game_data <- split(team_games, team_games$team)
#   
#   return(team_game_data)
# }
# 
# # Function to reshape the data to one row per team game
# reshape_team_games <- function(games_with_conf) {
#   games_long <- games_with_conf %>%
#     pivot_longer(
...
#     ) %>%
#     mutate(
#       is_home_game = home_away == "home_team",
#       team_score = ifelse(is_home_game, home_score, away_score),
#       opponent_score = ifelse(is_home_game, away_score, home_score),
#       opponent = ifelse(is_home_game, away_team, home_team),
#       result = case_when(
...
#         TRUE ~ NA_character_
#       ),
#       conference_game = ifelse(is_home_game, home_team_conf == away_team_conf, away_team_conf == team_conf),
#       div_game = ifelse(is_home_game, home_team_div == away_team_div, away_team_div == team_div)
#     ) %>%
#     select(team, week, opponent, result, conference_game, div_game, gameday)
#   
#   return(games_long)
# }
# 
